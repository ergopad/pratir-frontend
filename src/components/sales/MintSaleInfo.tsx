...
const MintSaleInfo: FC<{
  ...
  const getPriceAndCurrency = (pack: IPack) => {
    const isSigUSD =
      pack.price[0].tokenId ===
      "03faf2cb329f2e90d6d23b58d91bbb6c046aa143261cc21f52fbe2824bfcbf04";
    const price =
      pack.price[0].amount < 0.1
        ? pack.price[0].amount
        : isSigUSD
        ? Number(
            (pack.price[0].amount * 0.01).toLocaleString(undefined, {
              maximumFractionDigits: 2,
            })
          )
        : Number(
            (pack.price[0].amount * 0.000000001).toLocaleString(undefined, {
  ...
  useEffect(() => {
    if (apiGetSaleById?.packs?.length > 0) {
      ...
      const initialPackId =
        openNow && apiGetSaleById.packs.length >= 3
          ? apiGetSaleById.packs[2].id
          : apiGetSaleById.packs[0].id;
      const initialSoldOut =
        openNow && apiGetSaleById.packs.length >= 3
          ? apiGetSaleById.packs[2].soldOut
          : apiGetSaleById.packs[0].soldOut;
      const initialDerivedPrices =
        openNow && apiGetSaleById.packs.length >= 3
          ? apiGetSaleById.packs[2].derivedPrice
          : apiGetSaleById.packs[0].derivedPrice;
    ...
    if (selectedIndex !== -1) {
      ...
      if (apiGetSaleById.packs[packIndex]) {
        const { price, currency } = getPriceAndCurrency(
          apiGetSaleById.packs[packIndex]
        );

        // console.log(apiGetSaleById.packs[packIndex], price);

        const packId = openNow
          ? apiGetSaleById.packs[packIndex + 2]?.id
          : apiGetSaleById.packs[packIndex]?.id;
        const soldOut = openNow
          ? apiGetSaleById.packs[packIndex + 2]?.soldOut
          : apiGetSaleById.packs[packIndex]?.soldOut;
        const currentDerivedPrices = openNow
          ? apiGetSaleById.packs[packIndex + 2]?.derivedPrice
          : apiGetSaleById.packs[packIndex]?.derivedPrice;
    ...
    <>
      ...
      >
        <Box>
          ...
          <Typography variant="body2">{apiGetSaleById.description}</Typography>
        </Box>
        <Box>
          <UserMenu />
        </Box>
          ...
          {apiGetSaleById !== undefined && apiGetSaleById.packs.length > 3 && (
            <>
              <Paper sx={{ mb: 2, p: 2 }}>
                ...
                {apiGetSaleById.packs
                  .filter((_item, i) => i % 3 === 0)
                  .map((item, i) => {
                    const packIndex = i * 3;
                    return (
                      <PackTokenSelector
              ...
              <Paper sx={{ mb: 2, p: 2 }}>
                ...
                >
                  {apiGetSaleById.packs
                    .filter((_item, i) => i % 3 === 1)
                    .map((pack, index) => {
                      return (
                        <Collapse key={index} in={selected[index]}>
                          {pack.content.map((content, i) => {
                            const totalOdds = content.rarity.reduce(function (
                              tot,
                              arr
                            ) {
                              return tot + arr.odds;
            ...
            apiGetSaleById.packs[1].content.map((content, i) => {
              ...
              return (
                <Paper sx={{ mb: 2, p: 2 }} key={i}>
                  ...
                  <Grid container justifyContent="space-between" sx={{ mb: 1 }}>
                    <Grid>
                      <Typography sx={boldTextSx}>Pack Contents:</Typography>
                    </Grid>
                    <Grid>
                      {content.rarity.length === 1 && (
                        <Typography color="text.secondary" sx={textSx}>
