...
import { getAssetInfo, resolveIpfs } from "@lib/utilities/assetsNew";
...
const TokenInfo: FC<{
  ...
  const [v2MetaData, setV2MetaData] = useState<JsonObject | undefined>(
    undefined
  );
  ...
  const fetchData = async (id: string) => {
    ...
    if (data.extraMetaData?.standard === 2) {
      setCollection({
        ...
        link:
          "/collections/" + data.extraMetaData.standard2Data?.collection?.name,
      });
      const traits = data.extraMetaData.standard2Data.traits;
      const flatTraits: Record<string, any> = Object.keys(traits).reduce(
      ...
      const explicitValue =
        data.extraMetaData.standard2Data.additionalInfo.find(
          (item: any) => item.key === "explicit"
        );
      if (explicitValue) {
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Typography variant="body2">{tokenDetails.description}</Typography>
                ...
                <Grid container justifyContent="space-between" sx={{ mb: 1 }}>
                  ...
                  <Grid item>
                    <Typography color="text.secondary" sx={textSx}>
                      <Link href={collection.link}>{collection.name}</Link>
                    </Typography>
                  </Grid>
                </Grid>
              )}
              {artist && (
                <Grid container justifyContent="space-between" sx={{ mb: 1 }}>
                  <Grid item xs="auto" sx={{ pr: 3 }}>
                    <Typography sx={boldTextSx}>Artist:</Typography>
                  </Grid>
                  <Grid item zeroMinWidth xs>
                    <Typography
                      color="text.secondary"
                      sx={{ ...textSx, textAlign: "right" }}
                      noWrap
                    >
                      <Link href={artist.link}>{artist.name}</Link>
                    </Typography>
                  </Grid>
                </Grid>
              )}
          ...
          {v2MetaData !== undefined && Object.keys(v2MetaData).length !== 0 && (
            <Card sx={{ mb: 2 }}>
              <CardContent sx={{ pb: "8px!important" }}>
                <Typography variant="h5">V2 Metadata</Typography>
          ...
          {tokenDetails.metaData !== undefined &&
            Object.keys(tokenDetails.metaData).length !== 0 && (
              <Card sx={{ mb: 2 }}>
                <CardContent sx={{ pb: "8px!important" }}>
