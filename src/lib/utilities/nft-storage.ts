...
export const storeNFT = (image: File) => {
  ...
  if (NFT_STORAGE_KEY === undefined)
    return console.error(
      "Failed to upload to nft.storage: No API key in environment file."
    );
  else {
...
export const ipfsUpload = async (
  ...
  try {
    ...
    const ipfs = res.data.url.replace(
      "https://blitztcg.myfilebase.com/ipfs/",
      "ipfs://"
    );
